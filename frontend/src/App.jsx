            ...
            {[...models,{name:'Average',fv:avgFV,sub:'Consensus',highlight:true}].map(m=>{
              ...
              return (
                <div key={m.name} style={{...C.card,...(m.highlight?{borderColor:'var(--accent)',boxShadow:'var(--shadow-glow)'}:{})}} className="p-4">
                  ...
                  {up!=null&&(<><div className="text-sm font-semibold mt-0.5 num" style={{color:up>=0?'var(--green)':'var(--red)'}}>{up>=0?'+':''}{fmt(up,1)}%</div><div className="mt-1"><Badge upside={up}/></div></>)}
                </div>
              );
            })}
          </div>
...
{data.history&&data.history.length>=3&&(()=>{
  const hist3 = data.history.filter(r=>r.fcf&&r.fcf>0).slice(-3);
  const norm = hist3.reduce((s,r)=>s+(r.fcf||0),0)/hist3.length;
  const curr = data.financials.fcf;
  const diff = curr && norm ? ((curr/norm)-1)*100 : null;
  return(
    <div className="text-xs mb-3 px-3 py-2 rounded-lg" style={{background:'var(--bg-subtle)',color:'var(--text-muted)'}}>
      <span>DCF uses </span>
      <strong style={C.s}>Normalized FCF (3Y avg): {fmtB(norm)}</strong>
      {diff !== null && Math.abs(diff) > 10 && (
        <span style={{marginLeft: 8, color: diff > 0 ? 'var(--green)' : 'var(--amber)'}}>
          vs TTM {fmtB(curr)} ({diff > 0 ? '+' : ''}{fmt(diff, 0)}%)
        </span>
      )}
      <div style={{marginTop: 4, opacity: 0.8}}>Smoothing out one-time items for a more conservative estimate.</div>
    </div>
  );
